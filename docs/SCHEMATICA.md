...
| **DebugDrawerPacket** (Mojang script-debug overlay; `DebugBox` color + lifetime, batched shapes) | ✅ shipped v2 | Closest thing to real hologram on stock bedrock + proxy. Crisp boxes, no collision, server sees nothing. **Caveat:** codec refs `java.awt.Color` (absent on stock Android) — guarded: auto-falls back to particles with a chat notice if linkage fails. |
